 ...
 */
public class MealsFragment extends Fragment implements View.OnClickListener, OnNetworkCallBack {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        adapter = new MealsAdapter(activity, mealList);
        ...
        if (Network.isConnected(activity)) {
            ...
            Network.setRequestForMealList(activity, object, this);
        } else {
    ...
    private void hitapi(int filter) {
        ...
        if (filter != 4) {
            ...
            object.addProperty("userId", userId);
            object.addProperty("filterId", String.valueOf(filter));
            object.addProperty("latitude", String.valueOf(filter));
            object.addProperty("longitude", String.valueOf(filter));
            object.addProperty("page", String.valueOf(pageNumber));
            Network.setRequestForMealList(activity, object, this);
        }
    }
        ...
        switch (v.getId()) {
            ...
            case R.id.tv_no_data:
                if (Network.isConnected(activity)) {
                    ...
                    JsonObject object = new JsonObject();
                    object.addProperty("userId", userId);
                    object.addProperty("filterId", filter);
                    object.addProperty("page", String.valueOf(pageNumber));
                    Network.setRequestForMealList(activity, object, this);
                } else {
                    rl_no_data.setVisibility(View.VISIBLE);
    ...
    public void onSuccess(int requestType, boolean isSuccess, String msg, Object data) {
        if (requestType == Network.REQUEST_TYPE_MEAL_LIST) {
            progress.dismiss();
            if (pageNumber == 0) {
    ...
    class MealsAdapter extends RecyclerView.Adapter {
        ...
        public MealsAdapter(Activity activity, ArrayList<MealListBean> mealList) {
        ...
        public void onBindViewHolder(RecyclerView.ViewHolder h, final int position) {
            ...
            final String mealId = mealList.get(position).getMealId();
            String mealName = mealList.get(position).getTitle();
            String comment = "$" + mealList.get(position).getPrice() + "/packet";
            String comments = "Comments " + String.valueOf(mealList.get(position).getCommentCount());
        ...
        public int getItemViewType(int position) {
            return Integer.parseInt(mealList.get(position).getMealId());
        }
    }
    ...
    private void showFilterPopup(ImageView iv_options) {
        try {
            ...
            TextView popular = (TextView) layout.findViewById(R.id.popular);
